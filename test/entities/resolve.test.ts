...
test("new entity is enriched, linked, and gets an entity chunk", async () => {
  ...
  const [link] = await db.select().from(schema.itemEntities)
...
test("existing entity is reused: no candidate search, no verify, just a link", async () => {
  ...
  expect(await db.select().from(schema.itemEntities)).toHaveLength(2)
  ...
  const ep2Chunk = chunks.find((c) => c.itemId === ep2.id)
...
test("verifier rejection stores an unmatched entity that is still linked", async () => {
  ...
  expect(await db.select().from(schema.itemEntities)).toHaveLength(1)
}, 30_000)
